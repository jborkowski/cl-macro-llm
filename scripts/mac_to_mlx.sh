...
# CPU/MPS, then quantize to native Apple Silicon MLX 4-bit. End-to-end
# ~1-2 h on a 128 GB Mac. Requires ~120 GB free disk (54 GB cached base +
# 54 GB merged + 14 GB MLX 4-bit output).
#
# Prereqs:
#   - mlx-lm (pip install --upgrade mlx-lm)
#   - transformers v5 (pip install --upgrade transformers peft torch huggingface_hub)
...
step "1/3  Checking deps"
python -c "import mlx_lm" 2>/dev/null || fail "mlx-lm not installed. pip install --upgrade mlx-lm"
python -c "import transformers, peft" 2>/dev/null || fail "transformers/peft missing. pip install --upgrade transformers peft torch huggingface_hub"
TFV=$(python -c "import transformers; print(transformers.__version__)")
echo "  transformers: $TFV"
[[ "${TFV:0:1}" == "5" ]] || warn "transformers $TFV may not support Qwen3.6 (qwen3_5). Need v5+. Continuing anyway."
...
base = AutoModelForCausalLM.from_pretrained(base_id, torch_dtype=torch.bfloat16, token=token, trust_remote_code=False)
